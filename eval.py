...
def eval_net(encoder, seg_decoder, val_loader, threshold=0.5, epoch=None):
    ...
    resolution = val_loader.dataset.info_dict['resolution']
    ...
    with torch.no_grad():
        for data3D, gt_list in val_loader:
            ...
            spinal_cord_pred1 = seg_decoder(encoder(x.cpu()))

            spinal_cord_pred = spinal_cord_pred1[:,0,:,:].unsqueeze(1)
            loss_spinal_cord = criterion(spinal_cord_pred.cpu(), spinal_cord_mask.cpu())
            ...
            loss_gm = F.binary_cross_entropy_with_logits(spinal_mask_pred.cpu(), gm_gt_mask.cpu(),
                                                         pos_weight=gm_pos_weight.cpu())
            gm_pred_mask = torch.sigmoid(spinal_mask_pred) > 0.5
            ...
            eval_result['auc'] += skmetrics.roc_auc_score(gm_gt_mask.view(-1).cpu().numpy(),
                                                          spinal_mask_pred.view(-1).cpu().numpy())
        ...
        for key, val in eval_result.items():
            eval_result[key] = val / len(val_loader)
...
if __name__ == '__main__':
    ...
    encoder = Encoder(c=1, norm='bn', activation='relu')
    seg_decoder = Decoder(num_classes=2, norm='bn', activation='relu')
    state_dicts = torch.load('../outdir/scgm/target1/final_model.pth')
    ...
    val_loader = DataLoader(target_domain_dataset, batch_size=1, shuffle=False,
                            pin_memory=True)
    eval_net(encoder, seg_decoder, val_loader)
