...
class SCGM(Dataset):
    ...
    def __getitem__(self, index):
        ...
        train_domain_name.remove(self.domain_name[self.test_domain_idx])
        id = self.id_path[index]
        img = np.load(os.path.join(self.base_dir, id))['arr_0']
        mask = np.load(os.path.join(self.base_dir, id.replace('image', 'mask')))['arr_0']
        mask = mask[:, :, 1]
        ...
        if self.split == 'test':
            ...
            img = transform(img)
            mask = transform(mask)
            img = F1.to_tensor(np.array(img))
            mask = F1.to_tensor(np.array(mask))
            mask = (mask > 0.1).float()
            ...
            img = F1.rotate(img, angle, InterpolationMode.BILINEAR)
            mask = F1.rotate(mask, angle, InterpolationMode.NEAREST)
            if h > 110 and w > 110:
                size = (100, 100)
                transform_list = [transforms.CenterCrop(size)]
                transform_list = [transforms.Resize((self.newsize, self.newsize))] + transform_list
